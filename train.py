...
from nrdk.config import autoscale_batch_size, configure_rich_logging
...
def train(cfg: DictConfig) -> None:
    ...
    _log_level = configure_rich_logging(
        cfg.meta.get("verbose", logging.INFO),
        rank0_only=["train", "NRDKLightningModule"])
    ...
    if cfg["meta"]["name"] is None or cfg["meta"]["version"] is None:
        ...
        return
    cfg["datamodule"]["batch_size"] = autoscale_batch_size(
        cfg["datamodule"]["batch_size"],
        accumulation=cfg["trainer"].get("accumulate_grad_batches", 1))

    if cfg["meta"]["dry_run"] == "inst":
        cfg["trainer"]["accelerator"] = "cpu"
        cfg["trainer"]["devices"] = 1
    elif cfg["meta"]["dry_run"] == "full":
        with open_dict(cfg["trainer"]):
            cfg["trainer"]["fast_dev_run"] = True
            cfg["datamodule"]["batch_size"] = min(
                3, cfg["datamodule"]["batch_size"])
    ...
    if cfg["meta"]["dry_run"] == "inst":
        logger.info(
            "Dry run (inst): instantiation succeeded; skipping training.")
        return

    lightningmodule = lightningmodule.to(memory_format=torch.channels_last)

    if cfg["meta"]["recompile_limit"] is not None:
        torch._dynamo.config.recompile_limit = cfg["meta"]["recompile_limit"]
    if cfg["meta"]["compile"]:
        lightningmodule.compile(dynamic=False)
    ...
    if trainer.is_global_zero and cfg["meta"]["dry_run"] is None:
        meta: dict[str, Any] = {
            "duration": duration,
